...
//test
export const siteApi = "https://testadmin.sharifoilco.com/api";
export const siteApiUrl = "https://testadmin.sharifoilco.com";
...
const env = {
  ...
  kaveNegarApi:
    "464D534974436D623637454B5547393169306E4D557A59637030574F4C37314930567354446F52424938773D",
  ...
  ListFiltersApi: "/web/product/list-filters",
  ListProductsApi: "/web/product/list-product",
  FetchProductApi: "/web/product/fetch-product/",
  paymentApi: "/payment/",
...
export function comparePrice(price1, price2) {
  if (!price1) return 1;
  try {
    var rawPrice1 = parseInt(
      price1.toString().replace(/,/g, "").replace(/^\D+/g, "")
    );
    var rawPrice2 = parseInt(
      price2.toString().replace(/,/g, "").replace(/^\D+/g, "")
    );
...
export function discountPrice(priceText, discountPrice) {
  ...
  return priceTemp.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}
...
export function cartPrice(priceText, count) {
  if (priceText === null || priceText === undefined) return priceText;
  return (parseInt(priceText.toString().replace(/^\D+/g, "")) / count / 10)
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}
export function standardSku(sku) {
  let outSku = sku;
  try {
    outSku = sku.replace(/\//g, "--");
  } catch {}
  return outSku;
}
export function validPhone(phone_number) {
  if (phone_number.length < 10) return 0;
  return 1;
}
export function filterToUrl(filters) {
  const filterList = JSON.stringify(filters.filter)
    .replace("[", "")
    .replace("]", "");
  const catList = JSON.stringify(filters.category)
    .replace("[", "")
    .replace("]", "");
