...
function ProductTableRow(props) {
  ...
  var stockIndex = stockId
    ? product.countTotal.findIndex((item) => item.Stock == stockId.StockID)
    : -1;
  if (newStockCount && stockIndex !== -1)
    newStockCount = newStockCount[stockIndex].quantity;
    ...
    <React.Fragment>
      <tr className={activeAcc ? "activeAccordion" : "accordion"}>
        ...
        <td>
          <div className="cu-avatar">
            <img
              src={
                product.thumbUrl
                  ? env.siteApiUrl + product.thumbUrl
                  : env.defaultProduct
              }
        ...
        <td>
          <div className="order-num">
            <p>
              {product.brandInfo && product.brandInfo[0]
        ...
        <td>
          <div className="order-num existSmall">
            {Count ? (
              <p>
                {Count.storeCount - Count.orderCount}
                {token.access !== "sale" && Count.count3 ? (
                  <small className="store3Small">انبار3: {Count.count3}</small>
                ) : (
                  <></>
                )}
              </p>
      ...
      {activeAcc ? (
        <tr className="sub-order">
          <td colSpan="9">
            <ProductQuickDetail product={product} />
          </td>
        </tr>
      ) : (
