  ...
  try {
    ...
    if (cart)
      return res
        .status(400)
        .json({
          message: `مشتری دارای سبد خرید با شماره ${cart.cartNo} می باشد`,
        });
    const quickCart = await quickCart.findOne({ userId });
    if (quickCart)
      return res
        .status(400)
        .json({
        ...
        const deleteResponse = await customer.findOne({ _id: ObjectID(userId) });
