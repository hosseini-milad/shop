...
function OrderPopUp(props) {
  ...
  const addToCart = (newData) => {
    ...
    fetch(
      env.siteApi + `/panel/${tab ? "quote" : "faktor"}/edit-addCart`,
      postOptions
    )
      .then((res) => res.json())
      .then(
  ...
  const removeItem = (rData) => {
    const postOptions = {
      ...
      body: JSON.stringify({ cartNo: data.orderNo, cartID: rData.cartID }),
    };
        ...
        <div className="modal-backdrop show-modal">
          <div className="task-popup fullPopUp">
            <div className="orderModalTitle">
              ...
              <sub>
                (
                {props.creator && props.creator[0]
                  ? props.creator[0].username
                  : "-"}
                )
              </sub>
              ...
              <main className="sharif-order-main">
                {content ? (
                  <QuickCartHolder
                    ...
                    cartDetail={
                      content.orderData ? content.orderData : content.cartDetail
                    }
                    setError={setError}
