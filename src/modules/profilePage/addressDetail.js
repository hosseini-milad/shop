...
function AddressDetail(props) {
  ...
  useEffect(() => {
    ...
    fetch(siteApi + "/auth/list-address", postOptions)
      ...
      .then(
        (result) => {
          if (result.data && result.data.length) setAddress(result.data);
        },
        (error) => {
          console.log(error);
        }
  ...
  const handleSubmit = (event) => {
    ...
    body &&
      fetch(siteApi + env.userEditAddress + event.target[0].value, postOptions)
        .then((res) => res.json())
        .then(
          (result) => {
  ...
  const onStateChange = (e) => {
    ...
    const body = {
      province_id: data.stateList.find((record) => record.name === e).id,
    };
    const postOptions = {
      method: "post",
      headers: {
          ...
          <div key={i}>
            <div className="profileTitle">
              <strong>
                {" "}
                - {address.name}
                <a className="addressDetail" onClick={() => setAddressTab(i)}>
                  مشاهده جزئیات
                </a>
              </strong>
              {selectAddress ? (
            ...
            <form
              ...
              style={{ display: addressTab === i ? "flex" : "none" }}
            ...
            >
              ...
              <input
                type="button"
                className="offerButton removeButton"
                value="حذف"
