import { normalPriceCalc, normalPriceCount, normalPriceRound } from "../../env";
import Num2persian from "num2persian";

var token = JSON.parse(localStorage.getItem("token-lenz"));

function OfficialPrintSepidar(props) {
  ...
  return (
    <div className="printArea officialprint">
      <div className="userInfo">
        ...
        <div
          className="hesabfaSection hesabfaDate"
          style={{ minWidth: "240px" }}
        >
          <small>
            شماره سریال: <b>{orderInfo.cartNo}</b>
          </small>
          <small>
            {" "}
            تاریخ:{" "}
            <b>
              {new Date(orderInfo.progressDate).toLocaleDateString("fa-IR")}
            </b>
          </small>
        </div>
      </div>
                ...
                <tbody>
                  ...
                  <tr>
                    <td colSpan={2} className="hesabfaItem">
                      <span>نشانی: </span>
                      <strong>
                        کرج-جاده قزلحصار-خیابان پارس لانه بتن-پلاك 15
                      </strong>
                    </td>
                    <td className="hesabfaItem">
                      <span>کدپستی: </span>
          ...
          {orderInfo &&
            ...
            orderInfo.cartItems.map((items, i) => (
              <tr key={i}>
                ...
                <td>{items && items.sku}</td>
                <td>{items && items.title}</td>
                <td className="centerCell number-td">{items.ProductPackQuantity}</td>
                <td className="centerCell">
                  {items.ProductPackRef ? items.ProductPackRef : "تعداد"}
                </td>
                <td className="centerCell number-td">{items.count}</td>
                <td className="number-td">
                  {normalPriceRound(items.total.price)}
                </td>
                <td className="number-td">
                  {normalPriceCount(items.total.price, items.count)}
                </td>
                <td className="number-td">
                  {normalPriceRound(items.total.discount)}
                </td>
                <td className="number-td">
                  {normalPriceCalc(
                    items.Addition,
                ...
                <td className="number-td">
                  {normalPriceRound(items.total.tax)}
                </td>
                <td className="number-td">
          ...
          <tr>
            <td colSpan={8}>
              <div className="table-total">
                <p>
                  {Num2persian(normalPriceRound(data.orderData.totalPrice))}{" "}
                  ریال
                </p>
                <p>جمع کل</p>
              </div>
            </td>
            <td className="number-td">{normalPriceRound(data.orderData.totalFee)}</td>
            <td className="number-td">{normalPriceRound(data.orderData.totalDiscount)}</td>
            <td className="number-td">{normalPriceRound(data.orderData.Addition)}</td>
            <td className="number-td"></td>
            <td className="number-td">{normalPriceRound(data.orderData.totalTax)}</td>
            <td className="number-td">{normalPriceRound(data.orderData.totalPrice)}</td>
          </tr>
        </tbody>
      </table>
      <div className="table-footer">
        <div className="footer-row">
          ...
          <div className="bill-col">
            <p>
              جمع کل حساب شما با احتساب این فاکتور <span>0</span>ریال میباشد
            </p>
          </div>
        </div>
        <div className="footer-row">
            ...
            <div className="account-wrapper">
              ...
              <div className="account-col">
                <div className="account-item">
                  ...
                  <p className="ac-num">
                    <span>IR</span>540120020000005570093050
                  </p>
                </div>
                <div className="account-item">
                  <p className="ac-name">شبا</p>
                  <p className="ac-num">
                    <span>IR</span>330150000000519302175702
                  </p>
                </div>
                <div className="account-item">
                  <p className="ac-name">شبا</p>
                  <p className="ac-num">
                    <span>IR</span>530190000000213343979001
                  </p>
                </div>
                <div className="account-item">
                  <p className="ac-name">شبا</p>
                  <p className="ac-num">
                    <span>IR</span>480170000000110989693003
                  </p>
                </div>
              </div>
            </div>
...
export default OfficialPrintSepidar;
